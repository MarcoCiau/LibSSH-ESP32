...
ssize_t sftp_read(sftp_file handle, void *buf, size_t count) {
  ...
  switch (msg->packet_type) {
    ...
    case SSH_FXP_DATA:
      ...
      if (datalen > count) {
        ssh_set_error(sftp->session, SSH_FATAL, "Received a too big DATA packet from sftp server: %u and asked for %u" , datalen, count);
  ...
  switch (msg->packet_type) {
    ...
    case SSH_FXP_DATA:
      ...
      if (ssh_string_len(datastring) > size) {
        ssh_set_error(sftp->session, SSH_FATAL, "Received a too big DATA packet from sftp server: %u and asked for %u", ssh_string_len(datastring), size);
        SSH_STRING_FREE(datastring);
        return SSH_ERROR;
      }
